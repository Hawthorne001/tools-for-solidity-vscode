...
export class PipxInstaller implements Installer {
            ...
            async () => {
                ...
                if (this.prerelease) {
                    this.outputChannel.appendLine(
                        `Running 'pipx upgrade --pip-args=--pre eth-wake'`
                    );
    ...
    startWake(port: number): ExecaChildProcess {
        ...
        return execa(`pipx run --spec eth-wake wake lsp --port ${port}`, {
            shell: true,
    ...
    async setup(): Promise<void> {
        ...
        try {
            ...
            if (compare(version, WAKE_MIN_VERSION) < 0) {
                this.outputChannel.appendLine(
                    `Found 'eth-wake' in version ${version} but the target minimal version is ${WAKE_MIN_VERSION}.`
                );
                await this.pipxUpgrade();
            }
            const wakeVersion = this.getWakeVersion();
            this.analytics.setWakeVersion(wakeVersion);
